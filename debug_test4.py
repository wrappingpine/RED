...
for i in range(15):
    ...
    print(f'Frame {i}: pinch_events={len(pinch_events)}, events={len(events)}, tracking_state={t.recognizer.state}')
    for e in pinch_events:
        print(f'  Event: {e.gesture_type}, state: {e.state}')
    # Also check state machine
    print(f'  State machine LEFT_CLICK state: {t.recognizer._state_machine.get_gesture_state(GestureType.LEFT_CLICK)}')